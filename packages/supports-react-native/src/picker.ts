...
async function fileSize(uri: string): Promise<number> {
  // Best-effort. Works in RN where fetch(file://) returns a blob.
  try {
...
function assetToInput(asset: {
  uri: string; mimeType?: string | null; width?: number; height?: number;
  duration?: number | null; fileSize?: number | null; type?: string | null;
}, kind: AttachmentKind, sizeBytes: number): AttachmentInput {
  return {
    uri: asset.uri,
...
export type PickOptions = {
  ...
  multiple?: boolean;
  /** Max images per pick when multiple is true. */
  selectionLimit?: number;
  /** JPEG quality 0–1 for images (default 0.85). Ignored for video. */
  quality?: number;
  /** Allow user to crop/edit before returning (default false). */
  allowsEditing?: boolean;
};

/** Open the OS image picker and return AttachmentInput[] ready for sendMessage. */
...
/** Open the OS picker for a single video and return AttachmentInput[]. */
export async function pickVideo(options: Omit<PickOptions, "multiple" | "selectionLimit"> = {}): Promise<AttachmentInput[]> {
  const ImagePicker = await loadPicker();
  const perm = await ImagePicker.requestMediaLibraryPermissionsAsync();
  if (!perm.granted) throw new Error("Photo library permission denied");
...
/** Launch the camera and return a single AttachmentInput (image or video). */
...
/**
 * One-call convenience: open the picker, then send the chosen images as a
 * message in the active conversation. Returns the picked attachments (may be
 * empty if the user cancelled).
 *
 * Example:
 *   const { send } = useConversation();
 *   await pickAndSendImage({ send, text: "Here's the receipt" });
 */
...
/** Same as pickAndSendImage but for a single video. */
...
/** Same as pickAndSendImage but uses the camera. */
